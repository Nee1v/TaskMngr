...
def seed_data():
    db = SessionLocal()
    try:
        # 1. Clear ALL existing tasks and links
        db.query(Task).delete()
        # If using a secondary table for dependencies, we clear that too
        ...
        # ------------------ ORIGINS: BASIC TOOLS ------------------
        collect_shovel = Task(
            title="Collect shovel", 
            description="Locations : \n - In hallway where door unlock to gen 3 is\n - Immediately after spawning to the right of Maxis Brain, leaning on wall\n*NOTE* : Needed to dig up Ice Staff parts during snow weather",
            goal="Origins"
        )

        db.add(collect_shovel)
        all_tasks.append(collect_shovel)

        # ------------------ ORIGINS: ZOMBIE SHIELD ------------------
        s_handle = Task(
            title="Collect shield part (Handle)", 
            description="Locations (Gen 2 trenches) : \n - Inside wheelbarrow in the room on the right directly after exiting spawn room\n - End of gen 2 trenches in wheelbarrow\n - Top of Tank Station near large pipe", 
            goal="Origins"
        )
        
        s_visor = Task(
            title="Collect shield part (Visor)", 
            description="Locations (Gen 3 trenches) : \n - Bottom of Fire Tunnel directly outside spawn on left\n - Wheelbarrow in room opposite of Fire Tunnel\n - Underneath bridge next to Gen 3, part is next to crate", 
            goal="Origins"
        )
        
        s_frame = Task(
            title="Collect shield part (Frame)", 
            description="Locations (No Man's Land) : \n - Inside giant's right footprint near Gen 4\n - Inside giant's right footprint near Gen 5\n - Inside giant's right footprint next to the Mound", 
            goal="Origins"
        )

        build_shield = Task(
            title="Build Shield at worbench", 
            description="Workbenches are located at most major locations, wind tunnel workbench is the most effective", 
            goal="Origins"
        )

        shield_tasks = [s_handle, s_visor, s_frame, build_shield]
        all_tasks.extend(shield_tasks)
        db.add_all(shield_tasks)
        db.flush()
        ...
        generator_tasks = [gen1, gen2, gen3, gen4, gen5, gen6]
        db.add_all(generator_tasks)
        db.flush() 
        all_tasks.extend(generator_tasks)

        # ------------------ ORIGINS: MAXIS DRONE ------------------
        d_rotor = Task(
            title="Collect Maxis Drone Rotor", 
            description="Locations : \n - Top of mound in a wheelbarrow\n - Left of gramophone table at the middle level of excavation\n - Bottom level of excavation scaffolding", 
            goal="Origins"
        )
        d_brain = Task(
        ...
        collect_white_record = Task(
            title="Collect Blank Disk (White Record)", 
            description="Locations (Mound Area) : \n - Behind the excavation sign at the front of mound (Mound side that faces Gen 1/2/3)\n - Behind the mound on a wall near crates (Around church entrance path)\n - In a wheelbarrow at the very top of the mound", 
            goal="Origins"
        )

        db.add(collect_white_record)
        all_tasks.append(collect_white_record)

        # ------------------ ORIGINS: EXCAVATION ACCESS / GRAMAPHONE ------------------
        collect_gramophone = Task(
            title="Collect gramophone", 
            description="Location (Inside Mound/Excavation) : \n - On the floor in the middle level of excavation, next to bench", 
            goal="Origins"
        )

        place_gramophone = Task(
            title="Immediately place gramophone in Mound to unlock lower level of excavation", 
            description="Place it on the bench that it was lying next to in excavation to unlock lower level excavation (Where the staffs will be built)", 
            goal="Origins"
        )

        retrieve_gramophone = Task(
            title="Pick gramophone back up from middle level of excavation", 
            description="Once lower level of excavation is open, pick gramaphone back up. You will need this to enter the crazy place", 
            goal="Origins"
        )

        gram_tasks = [collect_gramophone, place_gramophone, retrieve_gramophone]
        all_tasks.extend(gram_tasks)
        db.add_all(gram_tasks)
        db.flush()
        ...
        # ------------------ ORIGINS: ICE STAFF ------------------
        ice_disk = Task(
            title="Collect Blue Disk (Ice Record)", 
            description="Locations (Gen 2 Tank Station):\n• Desk near the entrance\n• Shelf on the left after entering\n• Shelf in the back near the exit", 
            goal="Origins"
        )

        ice_p_spawn = Task(
            title="Collect Ice Staff part (Spawn area)", 
            description="Dig up dig sites during a SNOW round in the starting area (Gen 2 trenches / Gen 3 trenches). \n *NOTE* The staff piece will never come from first dig in a given area, multiple digs required", 
            goal="Origins"
        )
        ice_p_middle = Task(
            title="Collect Ice Staff part (No Man's Land)", 
            description="Dig up dig sites during a SNOW round in no mans land (Gen 4 / Gen 5). \n *NOTE* The staff piece will never come from first dig in a given area, multiple digs required", 
            goal="Origins"
        )
        ice_p_church = Task(
            title="Collect Ice Staff part (Church area)", 
            description="Dig up dig sites during a SNOW round in the Church or Gen 6 area. \n *NOTE* The staff piece will never come from first dig in a given area, multiple digs required", 
            goal="Origins"
        )

        ice_base_tasks = [ice_disk, ice_p_spawn, ice_p_middle, ice_p_church]
        all_tasks.extend(ice_base_tasks)
        db.add_all(ice_base_tasks)
        db.flush()
        ...
        ice_graves = Task(
            title="Freeze & Shoot 3 Gravestones", 
            description="Locations : \n - Near right giant footprint (Gen 4)\n - Near right giant footprint (Mound)\n - Tank path outside Tank Station (Up the mud path)", 
            goal="Origins"
        )
        ...
        ice_upgrade_tasks = [ice_portal, build_ice, ice_puzzle, ice_graves, ice_orb, ice_altar, ice_portal_exit, retrieve_gram_ice]
        all_tasks.extend(ice_upgrade_tasks)
        db.add_all(ice_upgrade_tasks)
        db.flush()
        ...
        # ------------------ ORIGINS: WIND STAFF ---------------------
        wind_disk = Task(
            title="Collect Yellow Disk (Wind Record)", 
            description="Locations (Gen 5 area) : \n - On the wall next to Stamin-Up\n - On the crates next to Gen 5\n - Directly to the left after entering the Gen 5 tunnel on bench (Lightning tunnel)", 
            goal="Origins"
        )
        ...
        wind_base_tasks = [wind_disk, wind_p_spawn, wind_p_nml, wind_p_church]
        all_tasks.extend(wind_base_tasks)
        db.add_all(wind_base_tasks)
        db.flush()

        wind_portal = Task(
            title="Enter Wind Portal & Retrieve Gem", 
            description="Place gramophone at Gen 4 tunnel. Retrieve the Yellow Gem from wind pedestal and exit. *NOTE* You may have left gramophone somewhere else (Go pick it up)", 
            goal="Origins"
        )
        ...
        wind_upgrade_tasks = [wind_portal, build_wind, wind_puzzle, wind_smoke, wind_orb, wind_altar, wind_portal_exit, retrieve_gram_wind]
        all_tasks.extend(wind_upgrade_tasks)
        db.add_all(wind_upgrade_tasks)
        db.flush()
        ...
        light_disk = Task(
            title="Collect Purple Disk (Lightning Record)", 
            description="Locations (Gen 4 area) : \n - On wheelbarrow next to Gen 4\n - Bench next to Perk-a-Cola near Gen 4\n - Bottom of Gen 4 Wind Tunnel", 
            goal="Origins"
        )
        ...
        light_base_tasks = [light_disk, light_p_tank1, light_p_tank2, light_p_tank3]
        all_tasks.extend(light_base_tasks)
        db.add_all(light_base_tasks)
        db.flush()

        light_dials = Task(
            title="Adjust all 7 lightning dials", 
            description="Turn dials to these positions : \n - Gen 1 : LEFT\n - Tank Station ( Next to Gen 2) : DOWN\n - Gen 4 : UP\n - Mound (Near church entrance against mound) : UP\n - Gen 5 : DOWN\n - Lower Church : RIGHT\n - Upper Church : UP", 
            goal="Origins"
        )

        all_tasks.append(light_dials)
        db.add(light_dials)
        db.flush()

        light_portal = Task(
            title="Enter Lightning Portal & Retrieve Gem", 
            description="Place gramophone at Gen 5 tunnel. Retrieve the Purple Gem from lightning pedestal and exit. *NOTE* You may have left gramophone somewhere else (Go pick it up)", 
            goal="Origins"
        )
        build_light = Task(title="Build Lightning Staff", description="Build at the Purple Altar in the lower excavation.", goal="Origins")
        light_puzzle = Task(
            title="Solve Lightning Puzzle (Crazy Place)", 
        ...
        light_upgrade_tasks = [light_portal, build_light, light_puzzle, light_orb, light_altar, light_portal_exit, retrieve_gram_light]
        all_tasks.extend(light_upgrade_tasks)
        db.add_all(light_upgrade_tasks)
        db.flush()
        ...
        # ------------------ ORIGINS: FIRE STAFF ------------------
        fire_disk = Task(
            title="Collect Red Disk (Fire Record)", 
            description="Locations (Gen 6 / Church area) : \n - In between the front left of tank, a crate, and the stone pillar\n - Upper church on a bench\n - Next to Gen 6 sign above a skeleton", 
            goal="Origins"
        )
        fire_p_plane = Task(
            title="Collect Fire Staff part (Flaming Plane)", 
            description="Shoot down the glowing orange plane in the sky (appears the round after opening the Church entrance). Pick up the part near the Mound/Gen 4 path.", 
            goal="Origins"
        )
        fire_p_gen6 = Task(
            title="Collect Fire Staff part (Gen 6 Chest)", 
            description="Capture Generator 6 and collect the part from the reward chest behind the church.", 
            goal="Origins"
        )
        fire_p_panzer = Task(
            title="Collect Fire Staff part (Round 8 Panzer)", 
            description="Kill the Panzer Soldat (first spawns Round 8) and pick up the part he drops. *Note* Use one of the staffs to kill Panzer for a much easier fight", 
            goal="Origins"
        )

        fire_base_tasks = [fire_disk, fire_p_plane, fire_p_gen6, fire_p_panzer]
        all_tasks.extend(fire_base_tasks)
        db.add_all(fire_base_tasks)
        db.flush()

        fire_p_gen6.depends_on.append(gen6)

        fire_portal = Task(
            title="Enter Fire Portal & Retrieve Gem", 
            description="Place gramophone at Gen 1 (Spawn) tunnel. Retrieve the Red Gem from fire pedestal and exit. *NOTE* You may have left gramophone somewhere else (Go pick it up)", 
            goal="Origins"
        )
        build_fire = Task(
            title="Build Fire Staff", 
            description="Build at the Red Altar in the lower excavation.", 
            goal="Origins"
        )
        fire_sacrifice = Task(
            title="Kill Zombies on Grates (Crazy Place)", 
            description="Stand on the metal grates near the 4 fire cauldrons in the Crazy Place. Kill zombies with the Fire Staff until all 4 cauldrons are lit.", 
            goal="Origins"
        )
        fire_puzzle = Task(
            title="Solve Fire Puzzle (Church)", 
            description="Go to the church basement. Match the symbols on the wall to the 4 glowing torches upstairs. (IN PROGRESS)", 
            goal="Origins"
        )
        fire_orb = Task(
            title="Align Gems & Shoot Red Orb", 
            description="Rotate all the rings in lower excavation to RED using the glowing levers in lower excavation, then shoot the red orb at the bottom with fire staff.", 
            goal="Origins"
        )
        fire_altar = Task(
            title="Place Staff in Fire Altar", 
            description="Return to the Crazy Place and place the fire staff in the red pedestal.", 
            goal="Origins"
        )
        fire_portal_exit = Task(
            title="(FIRE) Exit crazy place *READ DESC*", 
            description="At this point you can either:\nA. Leave the crazy place and upgrade all the staffs at the same time later.\nB. Upgrade the fire staff by getting kills in the crazy place while it is in the altar", 
            goal="Origins"
        )
        retrieve_gram_fire = Task(
            title="Pick up gramophone (FIRE tunnel) (Gen 1)", 
            description="Make sure to pick up gramophone from bottom of gen 1 tunnel.", 
            goal="Origins"
        )

        fire_upgrade_tasks = [fire_portal, build_fire, fire_sacrifice, fire_puzzle, fire_orb, fire_altar, fire_portal_exit, retrieve_gram_fire]
        all_tasks.extend(fire_upgrade_tasks)
        db.add_all(fire_upgrade_tasks)
        db.flush()
        ...
        soul_boxes = Task(
            title="Fill 4 Soul Boxes", 
            description="Soul box looks like a black box with pruple designs.\nFill boxes at :\n - Left church giant footprint\n - Gen 5 giant right footprint\n - Mound giant left footprint\n - Gen 4 giant right footprint", 
            goal="Origins"
        )

        one_inch_punch = Task(
            title="Collect One Inch Punch", 
            description="Pick up your reward from the orange glowing chest at Gen 1 or Gen 6.", 
            goal="Origins"
        )

        all_tasks.extend([soul_boxes, one_inch_punch])
        db.add_all([soul_boxes, one_inch_punch])
        db.flush()
        ...
        gs_tasks = [gs_collect, gs_place, gs_cleanse, gs_carry, gs_bench, gs_kills, gs_retrieve]
        all_tasks.extend(gs_tasks)
        db.add_all(gs_tasks)
        db.flush()
        ...
        upgrade_all_staffs = Task(
            title="Wield the Ultimate Power (Upgrade all staffs)", 
            description="All staffs must be in their pedestals in the Crazy Place. Get ~20 kills per staff in the Crazy Place until the staff icons on your HUD glow white. If you have already upgraded staffs in the previous steps use the upgraded staffs to upgrade the other staffs", 
            goal="Origins"
        )
        
        db.add(upgrade_all_staffs)
        db.flush()
        all_tasks.append(upgrade_all_staffs)

        upgrade_all_staffs.depends_on.extend([ice_altar, wind_altar, light_altar, fire_altar])

        place_fire_mound = Task(
            title="Place Fire Staff in Mound", 
            description="Place the upgraded Fire Staff into its slot at the very bottom of the Excavation Site (Pedestal in front of initial four staff pedestals).", 
            goal="Origins"
        )
        
        place_ice_church = Task(
            title="Place Ice Staff in Church Robot (Freya)", 
            description="Enter the Church giant (Freya) and place the Ice Staff in the pedestal.", 
            goal="Origins"
        )
        
        place_wind_nml = Task(
            title="Place Wind Staff in NML Robot (Odin)", 
            description="Enter the middle giant (Odin) and place the Wind Staff in the pedestal.", 
            goal="Origins"
        )
        
        place_light_spawn = Task(
            title="Place Lightning Staff in Spawn Robot (Thor)", 
            description="Enter the spawn giant (Thor) and place the Lightning Staff in the pedestal.", 
            goal="Origins"
        )

        final_placements = [place_fire_mound, place_ice_church, place_wind_nml, place_light_spawn]
        db.add_all(final_placements)
        db.flush()
        all_tasks.extend(final_placements)

        for task in final_placements:
            task.depends_on.append(upgrade_all_staffs)

        # ------------------ ORIGINS: RELEASING THE HORDE ------------------
        prep_airstrike = Task(
            title="Preparation for Horde Step", 
            description="If you have not already, go get all your perks and PAP gun.\nAll staffs will respawn at bottom of excavation, grab one (Fire and lightning are best).\n(!) Now go to Gen 5 and look outside the map for the concrete patch near the left footprint of the Middle Giant. Make note of this.\nYou will also need Airstrike grenades for next stap (The stone tablet subquest).", 
            goal="Origins"
        )
        press_button = Task(
            title="Press Red Button inside Robot *READ DESC*", 
            description="All 3 robots are now marching. You must press the button in robot head and throw the Airstrike grenades at concrete patch in quick succession.\nSOLO STRAT :\n 1. Enter the MIDDLE Giant (Odin) at the Gen 5 left or right footprint.\n 2. Wait for the countdown. Press the Red Button on '1'.\n 3. You will be ejected. Immediately sprint to the concrete seal and throw your Airstrike grenade.\nCo-op : \n 1. Have one player board any giant and one player on ground near concrete patch\n 2. Once ready, player in giant will press button and signal to ground player to throw airstrike grenade at concrete patch", 
            goal="Origins"
        )
        deploy_maxis_seal = Task(
            title="Deploy Maxis Drone at Seal", 
            description="Once the seal is broken by the Airstrike, deploy the Maxis Drone near the crater. He will fly in and unleash the Panzer horde.", 
            goal="Origins"
        )
        kill_panzers = Task(
            title="Kill the Panzer Horde", 
            description="As soon as Maxis enters the crater, multiple Panzers will drop into the map. Use upgraded staffs or a Ray Gun Mark II to clear them out quickly.", 
            goal="Origins"
        )

        rts_tasks = [prep_airstrike, press_button, deploy_maxis_seal, kill_panzers]
        all_tasks.extend(rts_tasks)
        db.add_all(rts_tasks)
        db.flush()
        ...
        get_zombie_blood = Task(
            title="Extinguish Flaming Carts (Zombie Blood)", 
            description="Use the ICE STAFF to shoot the fire on 3 flaming wagons around the Mound. A Zombie Blood power-up will spawn at the pack-a-punch side of the mound.\n*NOTE* Cannot be done during rainy weather.\n *NOTE 2* The following steps will be done in quick succession, read ahead to ensure you complete them within the duration of the zombie blood", 
            goal="Origins"
        )
        shoot_plane_ee = Task(
            title="Shoot down flaming plane", 
            description="While in Zombie Blood, look at the sky for a glowing red plane (similar to the fire part plane) and shoot it down.\n *NOTE* If you fail this step you will have to progress one round to obtain zombie blood from extinguished carts again", 
            goal="Origins"
        )
        kill_pilot_zombie = Task(
            title="Kill Invisible Pilot Zombie", 
            description="While still in Zombie Blood, run counter-clockwise around the mound. An invisible zombie (now visible while in zombie blood) will be running clockwise toward you. Kill him, he should drop the maxis drone.\n *NOTE* If you fail this step you will have to progress one round to obtain zombie blood from extinguished carts again BUT this time you dont need to shoot down plane", 
            goal="Origins"
        )
        collect_upgraded_drone = Task(
            title="Pick up upgraded Maxis Drone", 
            description="The pilot zombie will drop the upgraded Maxis Drone. Pick it up immediately.", 
            goal="Origins"
        )
        reclaim_drone = Task(
            title="Reclaim Maxis Drone from Bench", 
            description="Go back to the workbench where you originally crafted the drone and pick it up one last time.", 
            goal="Origins"
        )

        zombie_blood_ee_tasks = [get_zombie_blood, shoot_plane_ee, kill_pilot_zombie, collect_upgraded_drone, reclaim_drone]
        all_tasks.extend(zombie_blood_ee_tasks)
        db.add_all(zombie_blood_ee_tasks)
        db.flush()
        ...
        prep_fists = Task(
            title="Iron Fist Preparation", 
            description="Ensure all 6 generators are currently active and you have the basic 'One Inch Punch' equipped from soul box subquest.", 
            goal="Origins"
        )
        iron_fist_kills = Task(
            title="Melee Zombies at bottom of Mound", 
            description="Go to the very bottom of the Excavation site (near the staff pedestals). Melee zombies that have a white/misty glow around their arms. You must do this until a glowing white tablet appears.", 
            goal="Origins"
        )
        collect_iron_fist = Task(
            title="Pick up Upgraded Iron Fist", 
            description="Pick up the glowing white tablet to upgrade your melee to the elemental Iron Fist. This is required for the final step!", 
            goal="Origins"
        )

        fist_upgrade_tasks = [prep_fists, iron_fist_kills, collect_iron_fist]
        all_tasks.extend(fist_upgrade_tasks)
        db.add_all(fist_upgrade_tasks)
        db.flush()
        ...
        final_staff_deposit = Task(
            title="Deposit all Staffs in Crazy Place", 
            description="Go to the Crazy Place and place all 4 upgraded staffs into their respective pedestals. \n*WARNING*: Once you start the next step, you cannot leave until it is finished.\n Now is a good time to PAP all your weapons and get your gobblegum of choice", 
            goal="Origins"
        )
        final_soul_harvest = Task(
            title="Sacrifice Souls (Final Harvest)", 
            description="Kill zombies inside the Crazy Place using your weapons (Nukes don't count). You will need approximately 100 kills. The step is complete when the screen flashes white and the ceiling opens up.", 
            goal="Origins"
        )
        launch_maxis_final = Task(
            title="Send Maxis into the Light", 
            description="Deploy the Maxis Drone in the center of the Crazy Place. He will fly up into the light. Step into the center portal to trigger the final cutscene and complete the Easter Egg!", 
            goal="Origins"
        )

        final_ee_tasks = [final_staff_deposit, final_soul_harvest, launch_maxis_final]
        all_tasks.extend(final_ee_tasks)
        db.add_all(final_ee_tasks)
        db.flush()
